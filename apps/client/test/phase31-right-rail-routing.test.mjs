...
test("phase 31 commerce privilege and pass drop entrance navigation copy", () => {
  ...
  assert.match(privilegeSource, /"领取"/, "privilege page should expose daily manual claim action");
  assert.match(privilegeSource, /今日未领作废/, "privilege page should explain same-day expiry for manual claims");
  assert.doesNotMatch(privilegeSource, /game-privilege-eyebrow|game-privilege-side-ribbon/, "privilege page should not repeat owned/today labels in decorative chrome");
  assert.match(privilegeSource, /getPrivilegeRewardTitle/, "privilege cards should distinguish daily rewards from instant rewards");
  assert.match(source, /"长期有效 · 购买即得"/, "instant growth fund should explain permanent delivery");
  assert.match(source, /setShopNotice\(isPrivilegeProduct \? "已购买" : response\.data\.result\)/, "privilege purchase toast should be concise");
  assert.match(source, /setShopNotice\("今日已领取"\)/, "privilege claim toast should be concise");
