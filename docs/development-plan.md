...
- 跨服中心布局：
  - 使用全覆盖手游内页，不做模块化卡片堆叠。
  - 采用“赛事状态头 + 左侧模式轨 + 右侧当前赛事内容 + 内嵌关闭按钮”的跨服赛事页，不强制与聊天或邮件视觉一致。
  - 新增稳定测试钩子：`cross-server-unified-shell`、`cross-server-close-button`、`cross-server-mode-rail`、`cross-server-content-pane`。
  - 去掉 `CROSS 跨服中心` 英文大标题和后台式副标题；右上角关闭按钮不得与文字重叠。
  - 左侧模式轨固定为：赛季、榜单、商会、奖励、历史。
  - 默认进入“赛季”页，不把创业大赛、商会赛季、历史和规则全部堆在一个滚动面板里。
- 赛季页布局：
  - 顶部状态条：跨服分组、报名状态、我的排名、当前荣誉。
  - 主区域：今日跨服目标、赛季进度、下个奖励。
...
- 边界：
  - 第一批只做玩家端感知和布局增强：今日目标、赛季进度、奖励层级文案、冲榜助力说明和统一壳层。
