...
- 全覆盖内页复用规则：
  - 外部参考依据：参考《少年三国志》官方界面介绍中竖屏观看、顶部资源/VIP、活动/日常任务、征战、集市、邮件、聊天等功能入口的组织方式；参考《少年三国志》跨服争霸资料中“争霸赛主界面”、押注、报名资格、VIP 次数、排名奖励和全服奖励的赛事化表达；参考手游赛季通行证/限时事件资料中“挑战进度 + 免费/付费奖励轨 + 赛季期限”对留存和付费的承接方式。来源记录：`https://sg.youzu.com/wap/detail/strategy_new/5146732`、`https://sg.youzu.com/wap/detail/guide/129889`、`https://m.ouwan.com/news/snsgz20150518wallon2.html`、`https://www.gameres.com/861048.html`。
  - 聊天、邮件、跨服这类玩家主动进入的强功能页，优先使用“单一统一壳层 + 内嵌关闭按钮 + 左侧分类轨 + 右侧内容区”的结构，避免顶部大标题、独立说明条、多组筛选按钮和多张卡片纵向堆叠。
  - 统一壳层命名按功能建立稳定测试钩子，例如 `chat-unified-shell`、`mail-unified-shell`、`cross-server-unified-shell`；关闭按钮使用对应 `*-close-button`，分类轨使用 `*-channel-rail` 或 `*-mode-rail`，内容区使用 `*-content-pane`。
  - 左侧分类轨只承载高频一级分类，不放说明文案；右侧内容区承载当前分类的主任务、列表、详情、奖励和操作，保证玩家视觉上看到的是一个完整游戏面板，而不是多个后台模块。
